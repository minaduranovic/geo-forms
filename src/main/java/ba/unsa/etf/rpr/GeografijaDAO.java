...
public class GeografijaDAO {
    ...
    private GeografijaDAO() {
        ...
        try {
            drzavaUpit = conn.prepareStatement("SELECT drzava_id FROM drzava WHERE naziv=?");
        } catch (SQLException e) {
            regenerisiBazu();
            try {
                drzavaUpit = conn.prepareStatement("SELECT drzava_id FROM drzava WHERE naziv=?");
        ...
        try {
            gradUpit = conn.prepareStatement("SELECT grad_id, naziv, broj_stanovnika FROM grad WHERE drzava=?");
            ...
            vratiGradoveUpit = conn.prepareStatement("SELECT grad_id, naziv, broj_stanovnika, drzava FROM grad ORDER BY broj_stanovnika DESC");
            vratiDrzavuUpit = conn.prepareStatement("SELECT drzava_id, naziv, glavni_grad FROM drzava WHERE glavni_grad=?");
            ...
            vratiMaxIDGradUpit = conn.prepareStatement("SELECT MAX(grad_id)+1 FROM grad");
            vratiMaxIDDrzavaUpit = conn.prepareStatement("SELECT MAX(drzava_id) FROM drzava");
            izmijeniGradUpit = conn.prepareStatement("UPDATE grad SET naziv=?, broj_stanovnika=?, drzava=? WHERE grad_id=?");
            nadjiDrzavuUpit = conn.prepareStatement("SELECT drzava_id, naziv, glavni_grad FROM drzava WHERE naziv=?");
            vratiDrzavuNaOsnovuIDUpit = conn.prepareStatement("SELECT drzava_id, naziv, glavni_grad FROM drzava WHERE drzava_id=?");
            vratiSveDrzaveIzBazeUpit = conn.prepareStatement("SELECT drzava_id, naziv, glavni_grad FROM drzava ORDER BY naziv");
            obrisiGradIzBazeUpit = conn.prepareStatement("DELETE FROM grad WHERE grad_id=?");
            trenutniGradUpit = conn.prepareStatement("SELECT grad_id, naziv, broj_stanovnika, drzava FROM grad WHERE naziv=?");
        } catch (SQLException e) {
            e.printStackTrace();
        }
